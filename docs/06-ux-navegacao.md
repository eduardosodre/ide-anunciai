...
- **Perfil público igreja (layout Stitch):** rota `/perfil/igreja/{uuid}` segue o **mesmo shell visual** do ministro (hero full-bleed, grid 4+8, Material Symbols, copiar link com `btn-church-profile-share`), com chip fixo “Igreja”, cartão “Palavra” (Mateus 18:20), cartão “Conexão e localização” (local; telefone, e-mail e CEP só logados) e coluna “Sobre” com texto institucional + chip “Perfil institucional”. `usuario.foto_url` incluído na query pública da igreja para o avatar do hero.
